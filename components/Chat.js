...
export default class Chat extends Component {
  ...
  componentDidMount() {
    ...
    // Set up subscription to a default channel
    this.channel = this.pusher.subscribe("chat-room");

    // ??
    this.channel.bind("new-message", ({ chat = null }) => {
      const { chats } = this.state;
      chat && chats.push(chat); // If chat exist, chats.push. Else, nothing happens
      this.setState({ chats });
    });

    // Fetch fresh array of messages for the channel chat-room
  ...
  handleKeyUp = evt => {
    const value = evt.target.value; // chat box mesasge content
